...
ATPG::wptr ATPG::test_possible(const fptr fault) {
  ...
  if (fault->node->type != OUTPUT) {
    ...
    // cout << "iwire: "<< fault->node->iwire.front()->value <<endl;
    // cout << fault->node->iwire.front()->value <<endl;
...
bool ATPG::trace_unknown_path(const wptr w) {
  ...
  wptr wtemp;
	...
	//------------------------------------- hole ---------------------------------------
  //DFS
  if(w->onode[0]->type == OUTPUT)return true;
  if (w->value != U) return false;
  if (w->flag & OUTPUT) return true;
  for(auto &x:w->onode){ 
  	if (x->beDetected < current_DFS_num){
  			x->beDetected = current_DFS_num;
  			if(trace_unknown_path(x->owire.front())) 
  				return true; 
  	}
  	else 
    	return false;
  ...
  //   wptr _t = _q.front();
  //   _q.pop();
  //   if ( _t->value != U )continue;
  //   if ( _t->flag = OUTPUT) return true;
  //   for( auto &x:_t->onode ){
  //       _q.push(x->owire.front());
  //   } 
  // }
  // return false;
...
bool ATPG::check_test(void) {
  ...
  //TODO check if any fault effect reach PO
	...
	//--------------------------------- hole -------------------------------------------
  for(auto &x:cktout)
      if(x->value == D || x->value == B)  return true;
  // is_test = true;
	//----------------------------------------------------------------------------------
  //TODO
  return is_test;
